...
def setupTrace : List Action := [
  ...
  .draw 1, .draw 2, .draw 3,       -- draw 3 from shuffled discard (SoS resolved by autoDrain)
...
def loopTrace : List Action := [
  ...
  .draw 4, .draw 0,                -- draw Prep2(4), SoS(0) from drawPile
  .discard 2,                       -- discard Reflex (trigger: draw 3 bottom)
  .discard 1,                       -- discard Tact (trigger: +2E); resolveCard 3 auto -> Prep1 discard
  .draw 1, .draw 2, .draw 3,       -- shuffle [Prep1(3),Tact(1),Reflex(2)], draw 3
  .play 0,                          -- SoS: 4 hand cards -> 4 Shivs; autoDrain resolves all
  .draw 3, .draw 1, .draw 2,       -- shuffle discard, draw Prep1, Tact, Reflex
...
theorem proof : InfiniteCombo cardDB cards enemy := ComboStormOfSteel2Prep_infinite
