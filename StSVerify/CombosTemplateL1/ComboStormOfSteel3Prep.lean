/-
  StormOfSteel3Prep (Level 1: Existence of infinite loop)
  Cards: 6
...
/-
  Card reference (id, name, cost, type, effect):
    ...
    id= 3  早有准备+ #1 (PreparedPlus, 0E, Skill): Draw 2 cards. Discard 1 card.
    id= 4  早有准备+ #2 (PreparedPlus, 0E, Skill): Draw 2 cards. Discard 1 card.
    id= 5  早有准备+ #3 (PreparedPlus, 0E, Skill): Draw 2 cards. Discard 1 card.

  Initial state: all 6 cards in draw pile, 5 draws queued, 3 energy.
...
theorem ComboStormOfSteel3Prep_infinite : InfiniteCombo cardDB allCards enemy :=
