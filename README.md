...
| 钢铁风暴+3准备（6卡） | 6 | 静默猎手 | 720 种情况（120×6） | ✅ | ✅ |
| 钢铁风暴+打击（5卡） | 5 | 静默猎手 | 对手可困住准备+ | ✅ | ❌ |
| 发泄+不惧妖邪双输出 | 11 | 观者 | 48 种情况（24×2） | ✅ | ✅ |

- **L1**：12/12 已证明（单回合内循环，无需结束回合）
- **L2**：11/12 已证明为无限
   - 其中只有一个组合，即 [钢铁风暴+, 战术大师+, 本能反应+, 早有准备+, 打击] 模型在L1情况可以证明无限，在L2无法证明无限，同时也难以证明“不能无限”
...
**难点**：6张卡，2个洗牌点。Oracle 0 洗 5 张牌（120种排列），Oracle 1 洗 3 张牌（6种排列），共 720 种组合。

**锚定状态**：手牌 = {钢铁风暴, 战术大师, 本能反应, 准备②, 准备③}（5张），弃牌堆 = {准备①}，能量 = 6

**循环策略**（单回合内，无需结束回合）：

1. **打出钢铁风暴**（1费）→ 弃掉4张手牌（战术大师、本能反应、准备②、准备③），生成4把匕首。战术大师触发 +2费，本能反应触发抽3张
2. **从5张牌的洗牌堆抽3张**（Oracle 0 控制顺序，洗牌堆 = {战术大师, 本能反应, 准备②, 准备③, 准备①}）。**鸽巢原理：5张中有3张准备+，抽3张必定至少包含1张准备+**。剩余2张留在牌堆
3. **打出4把匕首**（0费）→ 16点伤害，匕首消耗
4. **打出一张准备+**（0费）→ 抽2张（从牌堆取回被对手藏起的2张牌），弃2张（弃掉战术大师和本能反应）。战术大师触发 +2费，本能反应触发抽3张
5. **从3张牌的洗牌堆抽3张**（Oracle 1 控制顺序，洗牌堆 = {本能反应, 战术大师, 钢铁风暴}）。**3张全部抽回，对手无法控制**
6. **回到锚定状态**：手牌 = {钢铁风暴, 战术大师, 本能反应, 准备+, 准备+}，弃牌堆 = {准备+}

**为什么对所有洗牌都成立（数学论证）**：
- **第2步（鸽巢原理）**：5张牌中3张是准备+，抽3张必定至少包含1张准备+，保证第4步有准备+可打出
- **第4步（准备+恢复）**：牌堆恰好剩2张（第2步留下的），准备+全部抽回。弃掉战术大师和本能反应（它们一定在手牌中，因为被打出的是准备+而非它们）
- **第5步（全量抽取）**：洗牌堆恰好3张 = 抽牌数，全部抽回，对手无控制权
- **`sameModAccum` 可互换性**：所有准备+具有相同的（名称, 费用, 伤害），无论哪张被藏在弃牌堆，排序后结果相同
...
4. **主定理层**：引入 oracle，组装桥接与验证。**无 `native_decide`**
